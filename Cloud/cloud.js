...
// instantiate
var APP_ID = 'TfBH3NJxzbOaxpksu5YymD4lP9bPlytcfZMG8i5a';
var REST_API_KEY = 'IYkecC2bwjTYy4suhkGTEn7yrJaLC7ygVc5Esedd';
		...
		'Vehicle', {
			...
			switch (parseInt(alert.lvl)){
				case AlertLevel.PARKED:
					var message = vehicle.make + ' ' + vehicle.model + ' has been parked!';
					updateStatus(alert, vehicle, message);
					break;

				case AlertLevel.MOVED:
					var message = vehicle.make + ' ' + vehicle.model + ' has been moved!';
					updateStatus(alert, vehicle, message);
					break;
				...
				case AlertLevel.STOLEN:
					var message = vehicle.make + ' ' + vehicle.model + ' has been stolen!';
					updateStatus(alert, vehicle, message);
				...
				case AlertLevel.CRASHED:
					var message = vehicle.make + ' ' + vehicle.model + ' has been crashed!';
					updateStatus(alert, vehicle, message);
					break;
			}
	    });
...
function updateStatus(alert, vehicle, message){
	...
	kaiseki.updateObject('Vehicle', vehicle.objectId, {
		'alertLevel': alertKey,
   		...
   		if (err)	//send notifications w.r.t. the alert level
